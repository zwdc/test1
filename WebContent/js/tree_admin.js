...
//		}
//	});
	return confirm("确认删除目录【" + treeNode.name + "】吗？");
}
...
function onRemove(event, treeId, treeNode) {
	$.ajax({
		async: false,			//同步，等待success完成后继续执行。
		cache: false,
		url: ctx+"/resource/whetherLock/lock",
		data: { id: treeNode.id },
		type: 'POST',
		dataType: "json",
		success: function(data){ 
...
function updateTreeSort(nodeId, NodeArr, showMsg) {
	...
	$.ajax({
		...
		data: { sortArr: sortArr.toString(), nodeArr: nodeArr.toString(), parentArr: parentArr.toString()  },
		type: 'POST',
		dataType: "json",
		success: function (data) {
