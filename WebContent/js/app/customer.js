...
$(function() {
	...
	partner_datagrid = $('#partner_datagrid').datagrid({
        url: ctx+"/partner/getList?tType=2",
        width : 'auto',
        ...
        columns : [ 
            [ 
              ...
              {field: 'fax', title: '传真', width: fixWidth(0.1), align: 'center'},
              {field: 'email', title: 'Email', width: fixWidth(0.1), align: 'center'}
    	    ] 
        ],
...
function showPartner(row) {
	var url_ = ctx+"/partner/toMain?tType=2";
	if(row) {
		url_ = ctx+"/partner/toMain?tType=2&infoId="+row.infoId;
	}
...
function showDetails(row){
	partner_dialog = $('<div/>').dialog({
        ...
        minimizable: true,
        maximizable: true,
        href: ctx+"/partner/details?tType=2&infoId="+row.infoId,
        onLoad: function () {
