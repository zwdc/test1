...
$(function() {
    ...
    model_datagrid = $('#model_datagrid').datagrid({
        url: ctx+"/processModel/listModel",
        width : 'auto',
		height :  fixHeight(0.92),
		pagination:true,
		rownumbers:true,
		border:false,
		singleSelect:true,
...
function add(){
    ...
    model_dialog = $('<div/>').dialog({
        ...
        minimizable: true,
        maximizable: true,
        href: ctx+"/processModel/toAddModel",
        buttons: [
            {
                text: '保存',
                iconCls: 'icon-save',
...
function deploy(){
	...
	if (row) {
        $.ajax({
    		async: false,
    		cache: false,
            url: ctx + '/processModel/deploy/'+row.id,
            type: 'post',
            dataType: 'json',
            data: {},
...
function exportmodel(){
	var row = model_datagrid.datagrid('getSelected');
	if (row) {
		window.open(ctx+"/processModel/export/"+row.id);
	} else {
		$.messager.alert("提示", "您未选择任何操作对象，请选择一行数据！");
	}
}
...
function del(){
	...
	if (row) {
        $.ajax({
    		async: false,
    		cache: false,
            url: ctx + '/processModel/delete/'+row.id,
            type: 'post',
            dataType: 'json',
            data: {},
    		success: function (data) {
